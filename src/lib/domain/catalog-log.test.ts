...
describe("catalog log", () => {
  it("records 250 houses as aufgenommen, Kern am 01.09., Erweiterung am 05.09.", () => {
    assert.equal(CATALOG_LOG.length, 250);
  ...
  it("counts stock at the selected instant", () => {
    ...
    assert.equal(housesAt(berlinMidnight(2026, 9, 6)), 250);
  });
  ...
  it("scopes the log to Tag / Monat / Jahr", () => {
    ...
    const extraDay = dashRange("day", "2026-09-05", new Date("2026-09-05T12:00:00+02:00"));
    assert.equal(catalogLogInRange(extraDay.from, extraDay.to).length, 200);
    const month = dashRange("month", "2026-09-05", new Date("2026-09-05T12:00:00+02:00"));
    assert.equal(catalogLogInRange(month.from, month.to).length, 250);
    const year = dashRange("year", "2026-09-05", new Date("2026-09-05T12:00:00+02:00"));
    assert.equal(catalogLogInRange(year.from, year.to).length, 250);
  ...
  it("filters Neu vs Geändert without rewriting official text", () => {
    ...
    assert.equal(filterCatalogLog(rows, "neu").length, 250);
    assert.equal(filterCatalogLog(rows, "geaendert").length, 0);
    assert.equal(filterCatalogLog(rows, "alle").length, 250);
